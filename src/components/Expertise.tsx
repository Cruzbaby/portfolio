...
const Expertise = () => {
  return (
    <section
      id="expertise"
      className="relative mx-auto mt-6 max-w-6xl overflow-hidden rounded-3xl border border-white/10 bg-white/5 px-6 py-14 shadow-2xl shadow-purple-500/10"
    >
      <div className="absolute inset-y-0 left-1/2 hidden w-px -translate-x-1/2 bg-gradient-to-b from-transparent via-white/10 to-transparent md:block" />
      <div className="space-y-4 text-center md:text-left">
        ...
        <p className="max-w-3xl text-base text-slate-300">
          A blend of engineering depth and product intuition. I help teams ship
          faster without sacrificing maintainability.
        </p>
      </div>

      <div className="mt-10 grid gap-8 md:grid-cols-3">
        {expertiseData.map(({ title, subtitle, icon, tags, accent }) => (
          <article
            key={title}
            className="group relative overflow-hidden rounded-2xl border border-white/10 bg-black/20 p-6 transition duration-300 hover:-translate-y-1 hover:border-white/30"
          >
            <div
      ...
      <Image
        ...
        width={720}
        height={240}
        className="pointer-events-none absolute -bottom-10 left-1/2 w-[70%] -translate-x-1/2 opacity-10 blur-sm"
      />
    </section>
  );
};
